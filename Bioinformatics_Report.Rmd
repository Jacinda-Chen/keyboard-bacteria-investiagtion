...
I used the packages dplyr and ggplot in order to visualize the data. I also tested between different sampling groups using Wilcox statistical tests in order to get the p value.
...
### Boxplot showering the colony abundances in each of my two treatments: Communal and Individual Computers, for the 1x dilution level
...
```{r abundances-stats}
# run a statistical test to compare the two gorups of abundances
culture_data %>%
  filter(student_name == "Jacinda Chen") %>%
  filter(dilution_1_10_or_100 == "1") %>%
  wilcox.test(data = .,
...
(Hard to interpret data/takes out human subjectiveness when looking for patterns that really aren't there)
...
**Figure 2:** Boxplot showing the number of morphotypes between communal and individual ocmputers, 1x dilution. The mean number of morphotypes were not significantly different between the communal and individual computers (Wilcox p = 0.09679).
...
**Table 1:** Table of Qubit DNA extraction samples of Key "F" series (cultured)

### Gel image with your lanes labeled (crop to only include your samples and ladder)


**Figure 3:** Gel image with lanes labeled

### Table with the locus of each sequence (16S), orignal filename of each sequence, the filename after correcting....

**Table 2:** Corrected/accepted sequences of samples
